...
describe("divide", function () {
  ...
  it("test failing scenario", function () {
    expect(new BigDecimal("509365950.27576", 18).div("0.003802940215600348").toString()).toBe(
      "133940036234.660966692739280406"
    );
  });
  ...
  it("should do basic floating point division big", function () {
    expect(new BigDecimal("29629629362962961839.48344234").div(12345678901234567890n).toNumber().toFixed(2)).toBe(
      "2.40"
    );
  });
  it("should do basic floating point division", function () {
  ...
  it("123456789.123456 / .0123456 = 10000063919.409020217", function () {
    expect(new BigDecimal("123456789.123456", 9).div(".0123456").toString()).toBe("10000063919.409020217");
  });
  // round up with precision 8
  it(".102 / .0383292 = 2.66115651", function () {
    expect(new BigDecimal(".102", 9).div(".0383292").toNumber().toFixed(8)).toBe("2.66115651");
  });
});
